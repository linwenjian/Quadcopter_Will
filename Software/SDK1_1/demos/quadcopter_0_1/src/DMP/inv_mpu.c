...
{
    uint32_t len_tmp = 1;
    len_tmp = (uint32_t)len;
    //MPU6050_Device.address = addr;
    I2C_DRV_MasterSendDataBlocking(0, &MPU6050_Device, &reg, 1, data, len, 100);
    return 0;
}
int8_t i2cread(uint8_t addr, uint8_t reg, uint8_t len, uint8_t * buf)
{
    ...
    I2C_DRV_MasterReceiveDataBlocking(0, &MPU6050_Device, &reg, 1, buf, len, 100);
    return 0;
}
...
int mpu_init(void)
{
    ...
    //i2c_read(0xD0,st.reg->who_am_i,1,&MPU_Test);
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    if (mpu_set_gyro_fsr(2000))	//gyro_cfg=0x18
        return -1;
    if (mpu_set_accel_fsr(2))	//accel_cfg=0x00
        return -1;
    if (mpu_set_lpf(42))       	//lpf=0x03
        return -1;
    if (mpu_set_sample_rate(50))//DIV=19  1000/(0x13+1)=50   lpf=0x04
        return -1;
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        ...
        if (st.chip_cfg.latched_int)
            tmp |= BIT_LATCH_EN | BIT_ANY_RD_CLR;
			//tmp |= BIT_BYPASS_EN; //我自己加进去的！！这样才可以使用辅助I2C设备5883！
        if (i2c_write(st.hw->addr, st.reg->int_pin_cfg, 1, &tmp))
            return -1;
    }
...
int mpu_set_dmp_state(unsigned char enable)
{
    ...
    if (enable) {
        ...
        mpu_set_sample_rate(st.chip_cfg.dmp_sample_rate);//st.chip_cfg.dmp_sample_rate=200
