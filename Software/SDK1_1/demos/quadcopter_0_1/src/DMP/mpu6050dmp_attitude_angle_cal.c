...
void attitude_angle_cal(int16_t* angle, int16_t* gyro, int16_t* accel)
{
    ...
    if (sensors & INV_WXYZ_QUAT)
    {
        ...
        q2=quat[2] / q30;
        q3=quat[3] / q30;
        My_Pitch = asin(-2 * q1 * q3 + 2 * q0 * q2) * 57.3f;                                                    //Pitch
        My_Roll  = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1) * 57.3f;                     //Roll
        ...
        if((My_Yaw-yaw_last) > 180.0f)
        {
                My_Yaw -= 360.0f;
        }
        else if((My_Yaw-yaw_last) < -180.0f)
        {
                yaw += 360.0f;
        }//预处理
        My_Yaw -= yaw_last;//将任意位置的偏航角调整为相对原始偏航角"yaw_last"的-180°~180°的数值。
#endif
